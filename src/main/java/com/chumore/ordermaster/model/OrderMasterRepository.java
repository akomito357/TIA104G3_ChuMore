...
public interface OrderMasterRepository extends JpaRepository<OrderMasterVO, Integer> {
	...
	@Query(value = "SELECT * FROM order_master om WHERE om.member_id = ? "
			+ "AND om.order_status = 1 ", countQuery = "SELECT COUNT(*) FROM order_master om WHERE om.member_id = :memberId "
					+ "AND om.order_status = 1 ", nativeQuery = true)
	...
	@Query(value = "SELECT DATE_FORMAT(om.served_datetime, '%Y-%m-%d %H:%i') as servedDatetime, ot.table_number as tableNumber, ot.order_table_id as orderTableId, "
			...
			+ "FROM order_master om " + "LEFT JOIN order_table ot on om.order_table_id = ot.order_table_id "
			+ "LEFT JOIN member m on om.member_id = m.member_id " + "WHERE om.rest_id = :restId "
			...
			+ "AND om.order_status = 1 ", countQuery = "SELECT COUNT(*) FROM order_master om "
					...
					+ "AND (IFNULL(m.member_name,'') LIKE CONCAT('%', :memberName, '%')) "
					+ "AND om.order_status = 1 ", nativeQuery = true)
	Page<Map<String, Object>> findOrderByRestId(Integer restId, LocalDateTime startDatetime, LocalDateTime endDatetime,
			Integer orderTableId, String memberName, Pageable pageable);

	@Query(value = "SELECT DATE_FORMAT(om.served_datetime, '%Y-%m-%d %H:%i') as servedDatetime, ot.table_number as tableNumber, ot.order_table_id as orderTableId, "
			+ "om.subtotal_price as subtotalPrice, om.order_id as orderId "
			+ "FROM order_master om " + "LEFT JOIN order_table ot on om.order_table_id = ot.order_table_id "
			+ "WHERE om.rest_id = :restId "
			+ "AND(:startDatetime IS NULL OR om.served_datetime >= :startDatetime) "
			+ "AND(:endDatetime IS NULL OR om.served_datetime <= :endDatetime) "
			+ "AND(:orderTableId IS NULL OR om.order_table_id = :orderTableId) "
			+ "AND om.order_status = 0 ", countQuery = "SELECT COUNT(*) FROM order_master om "
					+ "LEFT JOIN order_table ot on om.order_table_id = ot.order_table_id "
					+ "WHERE om.rest_id = :restId "
					+ "AND(:startDatetime IS NULL OR om.served_datetime >= :startDatetime) "
					+ "AND(:endDatetime IS NULL OR om.served_datetime <= :endDatetime) "
					+ "AND(:orderTableId IS NULL OR om.order_table_id = :orderTableId) "
					+ "AND om.order_status = 0 ", nativeQuery = true)
	Page<Map<String, Object>> findOrderByRestIdUnpaid(Integer restId, LocalDateTime startDatetime, LocalDateTime endDatetime,
			Integer orderTableId, Pageable pageable);

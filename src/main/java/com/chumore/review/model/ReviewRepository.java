...
public interface ReviewRepository extends JpaRepository<ReviewVO, Integer> {
    ...
    @Query("SELECT r FROM ReviewVO r WHERE r.restId = :restId ORDER BY r.reviewDatetime DESC")
    Page<ReviewVO> findByRestIdOrderByReviewDatetimeDesc(@Param("restId") Integer restId, Pageable pageable);

    @Query("SELECT r FROM ReviewVO r WHERE r.memberId = :memberId ORDER BY r.reviewDatetime DESC")
    List<ReviewVO> findByMemberIdOrderByReviewDatetimeDesc(@Param("memberId") Integer memberId);

    @Query("SELECT COUNT(r) > 0 FROM ReviewVO r WHERE r.orderId = :orderId")
    boolean existsByOrderId(@Param("orderId") Integer orderId);

    @Query("SELECT COUNT(r) FROM ReviewVO r WHERE r.restId = :restId")
    Long countByRestId(@Param("restId") Integer restId);

    @Query("SELECT AVG(r.reviewRating) FROM ReviewVO r WHERE r.restId = :restId")
